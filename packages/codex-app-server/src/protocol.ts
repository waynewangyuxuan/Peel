...
/**
 * Exhaustive stable server-to-client request surface generated by Codex
 * 0.154.0. The installed-schema check keeps this list honest when Codex is
 * upgraded; unknown future methods are still transported and rejected by the
 * host instead of being coerced into an unrelated response shape.
 */
export const STABLE_SERVER_REQUEST_METHODS = [
...
export type StableServerRequestMethod = typeof STABLE_SERVER_REQUEST_METHODS[number];
...
export function serverRequestRoute(method: string): ServerRequestRoute {
  if (!(STABLE_SERVER_REQUEST_METHODS as readonly string[]).includes(method)) return "unsupported-unknown";
  const stableMethod = method as StableServerRequestMethod;
  switch (stableMethod) {
    ...
    case "item/tool/call": return "unsupported-dynamic-tool";
    case "account/chatgptAuthTokens/refresh": return "unsupported-auth-refresh";
    case "attestation/generate": return "unsupported-attestation";
    case "applyPatchApproval": return "unsupported-legacy-file-approval";
    case "execCommandApproval": return "unsupported-legacy-command-approval";
    default: return assertUnreachable(stableMethod);
  }
}
...
function assertUnreachable(value: never): never {
  throw new Error(`Unclassified stable ServerRequest method: ${String(value)}`);
}
